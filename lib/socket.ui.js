		...
		onJob = function(job) {
			...
			var percentComplete = Math.round(job.status.progress.completed / job.status.progress.total * 100);

			var msg = '[JOB] (' + job.id + ') ' + job.status.state.is + ' - ' + job.status.state.message;
			msg+= '<div class="progress" style="height: 10px;"><div class="bar bar-success" style="height: 10px; width:' + percentComplete + '%;"></div></div>';
			//msg+= ' ' + Math.round(job.status.progress.completed / job.status.progress.total * 100) + '%';
			console.log(job);
			addMessage(msg);
		};
